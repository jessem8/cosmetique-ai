...
libraries are loaded only by ``GpuModelRuntime.start`` after their model snapshots
have been resolved.  The private API service uses this exact engine; there is no parallel runtime implementation.
"""
...
class RuntimeUnavailable(RuntimeError):
    """Raised when GPU cannot honestly advertise a ready V2 engine."""
...
DEFAULT_MODELS = (
    ModelSpec("grounding_dino", "IDEA-Research/grounding-dino-base", os.getenv("GPU_RUNTIME_GROUNDING_DINO_REVISION", "main")),
    # Transformers Sam2Model image segmentation requires the SAM 2.1 image
    # checkpoint. The older sam2-hiera-large repository exposes sam2_video
    # config and can collapse box prompts into tiny point-like masks.
    ModelSpec("sam2", "facebook/sam2.1-hiera-large", os.getenv("GPU_RUNTIME_SAM2_REVISION", "main")),
)
if os.getenv("GPU_RUNTIME_ENABLE_DFINE", "0").casefold() in {"1", "true", "yes"}:
    DEFAULT_MODELS = DEFAULT_MODELS + (
        ModelSpec("dfine", os.getenv("GPU_RUNTIME_DFINE_REPO", "ustc-community/dfine"), os.getenv("GPU_RUNTIME_DFINE_REVISION", "main"), required=False),
    )
...
class ModelRegistry:
    ...
    def resolve(self, token: str | None = None) -> dict[str, ResolvedModel]:
        try:
            ...
            raise RuntimeUnavailable("huggingface_hub is required for the GPU model registry") from exc
...
class GpuProductLockEngine:
    ...
    def _guided_refinement(
        ...
        for proposal in contaminants:
            ...
            if not (
                selected.x <= point.x <= selected.x + selected.width
                and selected.y <= point.y <= selected.y + selected.height
            ):
                continue
    ...
    def create_lock(
        ...
        *,
        ...
        chosen = (
            target_box
            if isinstance(target_box, NormalizedBox)
            else NormalizedBox.from_mapping(target_box)
            if target_box is not None
            else consensus.selected
        )
...
class GpuModelRuntime:
    ...
    def _load_detectors(self) -> None:
        ...
        self.engine = GpuProductLockEngine(proposal=proposal, sam2=sam2, dfine=dfine)
    ...
    def suspend_detectors(self) -> None:
        """Release detector objects if a future optional runtime needs it."""
        self.engine = None
        gc.collect()
        try:
            import torch
