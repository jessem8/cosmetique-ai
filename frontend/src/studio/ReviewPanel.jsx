...
export default function ReviewPanel({ sourceUrl, variantUrl, variant, finalCandidate, variants, selectedId, onSelect, onBack, onExport }) {
  const passed = finalCandidate?.qa?.passed === true || variant?.qa?.status === 'pass' || variant?.qa?.passed === true
  return (
      ...
      <div className="atelier-review__layout">
        ...
        <aside className="atelier-review__rail" aria-label="Vérification et export">
          ...
          <dl className="atelier-provenance">
             <div><dt>{STUDIO_COPY.review.model}</dt><dd>{finalCandidate?.model || variant?.provenance?.model || 'SDXL local'}</dd></div>
            <div><dt>{STUDIO_COPY.review.sourceHash}</dt><dd>{shortHash(variant?.provenance?.sourceSha256)}</dd></div>
             <div><dt>{STUDIO_COPY.review.outputHash}</dt><dd>{shortHash(finalCandidate?.checksum || variant?.provenance?.outputSha256)}</dd></div>
            <div><dt>Produit protégé</dt><dd>{passed ? 'Pixels vérifiés' : 'Vérification requise'}</dd></div>
          </dl>
