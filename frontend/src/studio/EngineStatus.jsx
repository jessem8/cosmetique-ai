...
export default function EngineStatus({ engine, onRefresh }) {
  ...
  return (
    <section className="atelier-engine" aria-labelledby="engine-title">
      ...
      <div className="atelier-engine__body"><h2 id="engine-title">{STUDIO_COPY.engine.title}</h2><strong>{checking ? STUDIO_COPY.engine.checking : ready ? STUDIO_COPY.engine.ready : engine.status === 'degraded' ? STUDIO_COPY.engine.degraded : STUDIO_COPY.engine.unavailable}</strong><p>{engine.message || (!ready && STUDIO_COPY.engine.unavailableHelp)}</p>{(engine.gpu || engine.runtime || engine.models) && <dl><div><dt>{STUDIO_COPY.engine.runtime}</dt><dd>{engine.runtime || 'Colab'}</dd></div><div><dt>{STUDIO_COPY.engine.gpu}</dt><dd>{engine.gpu || 'Non communiqué'}</dd></div><div><dt>{STUDIO_COPY.engine.models}</dt><dd>{Array.isArray(engine.models) ? engine.models.length : engine.models || 'Non communiqué'}</dd></div></dl>}</div>
