...
export default function EngineStatus({ engine, jobStatus, onRefresh }) {
  ...
  return (
    <section className="atelier-engine" aria-labelledby="engine-title">
      ...
      <div className="atelier-engine__body"><h2 id="engine-title">{STUDIO_COPY.engine.title}</h2><strong>{checking ? STUDIO_COPY.engine.checking : ready ? STUDIO_COPY.engine.ready : engine.status === 'degraded' ? STUDIO_COPY.engine.degraded : STUDIO_COPY.engine.unavailable}</strong><p>{engine.message || (!ready && STUDIO_COPY.engine.unavailableHelp)}</p>{jobStatus && <div className={`atelier-engine__job atelier-engine__job--${jobStatus.phase}`}><span>Job</span><strong>{jobStatus.label} · {jobStatus.progress === null ? 'En cours' : `${jobStatus.progress}%`}</strong><p>{jobStatus.detail}</p></div>}{(engine.gpu || engine.runtime || models.length) && <dl><div><dt>{STUDIO_COPY.engine.runtime}</dt><dd>{engine.runtime || 'Non communiqué'}</dd></div><div><dt>{STUDIO_COPY.engine.gpu}</dt><dd>{engine.gpu || 'Non communiqué'}</dd></div><div><dt>{STUDIO_COPY.engine.models}</dt><dd>{models.length ? models.join(', ') : ready ? 'Confirmés par le runtime' : 'Non communiqués'}</dd></div></dl>}</div>
